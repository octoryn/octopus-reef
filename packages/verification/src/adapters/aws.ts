  ...
  async retry(
    ...
    await this.#client.send(
      new ChangeMessageVisibilityCommand({
        ...
        ReceiptHandle: lease.receipt,
        VisibilityTimeout: seconds(
          Date.parse(availableAt) - Date.parse(this.#now()),
        ),
      }),
    );
  }
...
export class AwsS3VerificationStore implements SourceBundleStore {
  ...
  constructor(options: AwsS3VerificationStoreOptions) {
    ...
    this.#prefix =
      options.prefix?.replace(/^\/+|\/+$/g, "") ?? "reef-verification";
  }
  ...
  async descriptor(
    ...
    const bytes = await this.#get(
      this.#key(tenant, "source-descriptors", hash(sourceBundleRef)),
    );
    return JSON.parse(
  ...
  async putArtifact(
    ...
    await this.#put(
      this.#key(tenant, "artifacts", hash(ref)),
      content,
      mediaType,
    );
    return {
    ...
    await this.#client.send(
      new PutObjectCommand({
        ...
        ServerSideEncryption:
          this.#options.kmsKeyId === undefined ? "AES256" : "aws:kms",
  ...
  async #get(key: string): Promise<Uint8Array> {
    ...
    if (result.Body === undefined)
      throw new Error("S3 verification object has no body");
...
export class AwsS3VerificationArtifactStore implements VerificationArtifactStore {
  ...
  constructor(options: AwsS3VerificationStoreOptions | AwsS3VerificationStore) {
    this.#store =
      options instanceof AwsS3VerificationStore
        ? options
        : new AwsS3VerificationStore(options);
  }
  put(
    tenant: VerificationTenant,
    runRef: string,
    ...
    for (const binding of bindings) {
      ...
      if (secret === undefined) {
        const value = await this.#client.send(
          new GetSecretValueCommand({ SecretId: parsed.id }),
        );
        secret =
          value.SecretString ??
        ...
        if (secret === undefined)
          throw new Error(
            `verification profile secret is empty: ${binding.name}`,
          );
        cache.set(parsed.id, secret);
      }
      result[binding.environmentName] =
        parsed.field === undefined ? secret : jsonField(secret, parsed.field);
    }
    return result;
  }
}
...
class EcsVerificationSandbox implements VerificationSandbox {
  ...
  async execute(
    ...
    return {
      ...
      stdout: Buffer.from(value["stdoutBase64"], "base64"),
      stderr: Buffer.from(value["stderrBase64"], "base64"),
  ...
  async readFile(
    ...
    const value = (await request(
      this.fetchImpl,
      `${this.endpoint}/v1/files/read`,
      this.authToken,
      { path, maxBytes },
      signal,
    )) as Record<string, unknown>;
    if (value["found"] === false) return undefined;
    if (value["found"] !== true || typeof value["contentBase64"] !== "string")
      throw new Error("ECS sandbox read response is invalid");
...
export class AwsEcsVerificationSandboxProvisioner implements VerificationSandboxProvisioner {
  ...
  constructor(options: AwsEcsVerificationSandboxOptions) {
    if (options.subnets.length === 0 || options.securityGroups.length === 0)
      throw new Error(
        "private subnets and deny-by-default security groups are required",
      );
    if (Buffer.byteLength(options.runnerSharedSecret, "utf8") < 32)
      throw new Error("runner shared secret must contain at least 32 bytes");
    this.#options = options;
  ...
  async provision(
    ...
    const taskDefinition =
      this.#options.taskDefinitionByImageDigest[spec.imageDigest];
    if (taskDefinition === undefined)
      throw new Error("profile sandbox image digest is not registered for ECS");
    ...
    const started = await this.#client.send(
      new RunTaskCommand({
        ...
        overrides: {
          containerOverrides: [
            {
              name: this.#options.containerName ?? "verification-sandbox",
              environment: [
                ...
                {
                  name: "REEF_VERIFICATION_ATTEMPT",
                  value: String(spec.attempt),
                },
                {
                  name: "REEF_VERIFICATION_IMAGE_DIGEST",
        ...
        tags: [
          {
            key: "reef:organisation",
            value: hash(spec.organisationRef).slice(0, 24),
          },
    ...
    if (taskArn === undefined)
      throw new Error(
        `ECS verification sandbox failed to start: ${started.failures?.map((item) => item.reason).join(", ")}`,
      );
    const endpoint = await this.#wait(taskArn, signal);
    await ready(this.#options.fetchImpl ?? fetch, endpoint, token, signal);
    return new EcsVerificationSandbox(
      taskArn,
      endpoint,
      token,
  ...
  async destroy(sandbox: VerificationSandbox): Promise<void> {
    await this.#client.send(
      new StopTaskCommand({
        cluster: this.#options.cluster,
        task: sandbox.id,
        reason: "Reef deterministic verification finished",
      }),
    );
  }
  ...
  async #wait(taskArn: string, signal: AbortSignal): Promise<string> {
    ...
    while (Date.now() < deadline) {
      if (signal.aborted)
        throw signal.reason ?? new Error("verification provisioning aborted");
      const result = await this.#client.send(
        new DescribeTasksCommand({
          cluster: this.#options.cluster,
          tasks: [taskArn],
      ...
      if (task?.lastStatus === "RUNNING")
        return endpointFor(task, this.#options.port ?? 8081);
      if (task?.lastStatus === "STOPPED")
        throw new Error(
          `ECS verification sandbox stopped: ${task.stoppedReason ?? "unknown"}`,
        );
...
async function request(
  ...
  if (!response.ok)
    throw new Error(
      `ECS verification sandbox request failed: ${response.status} ${String(value.error ?? "")}`,
    );
  return value;
}
...
async function ready(
  ...
  const response = await fetchImpl(`${endpoint}/readyz`, {
    headers: { Authorization: `Bearer ${token}` },
    signal,
  });
  if (!response.ok)
    throw new Error(
      `ECS verification sandbox readiness failed: ${response.status}`,
    );
}
...
function endpointFor(task: Task, port: number): string {
  ...
  if (address === undefined)
    throw new Error("ECS verification sandbox has no private IPv4 address");
  return `http://${address}:${port}`;
}

function authToken(secret: string, spec: VerificationSandboxSpec): string {
  return createHmac("sha256", secret)
    .update(`${spec.runRef}\0${spec.attempt}\0${spec.imageDigest}`)
    .digest("base64url");
}
...
function parseMessage(body: string): VerificationQueueMessage {
  ...
  for (const key of [
    ...
    if (typeof value[key] !== "string" || value[key] === "")
      throw new Error(`invalid verification SQS message: ${key}`);
  }
  if (!Number.isInteger(value.attempt) || (value.attempt ?? 0) < 1)
    throw new Error("invalid verification SQS attempt");
...
function parseSecretRef(ref: string): { id: string; field?: string } {
  ...
  if (!ref.startsWith(prefix))
    throw new Error("unsupported verification profile secretRef");
  const value = ref.slice(prefix.length);
  const index = value.indexOf("#");
  const id = decodeURIComponent(index < 0 ? value : value.slice(0, index));
  const field =
    index < 0 ? undefined : decodeURIComponent(value.slice(index + 1));
  if (id === "" || field === "")
    throw new Error("invalid verification profile secretRef");
...
function jsonField(secret: string, field: string): string {
  ...
  if (value === null || typeof value !== "object" || Array.isArray(value))
    throw new Error("verification profile secret JSON is invalid");
  const selected = (value as Record<string, unknown>)[field];
  if (typeof selected !== "string")
    throw new Error("verification profile secret field is not a string");
  return selected;
}
...
function seconds(ms: number): number {
  return Math.max(0, Math.min(43_200, Math.ceil(ms / 1000)));
}
