...
export function createVerificationSandboxRunnerHandler(
  ...
  if (Buffer.byteLength(options.authToken, "utf8") < 32) {
    throw new Error(
      "verification sandbox auth token must contain at least 32 bytes",
    );
  }
  ...
  return (request, response): void => {
    if (request.method === "GET" && request.url === "/readyz") {
      json(response, 200, {
        ready: true,
        service: "reef-verification-sandbox",
      });
      return;
    }
    ...
    void readJson(request, options.maxBodyBytes ?? 16 * 1024 * 1024)
      .then(async (body) => {
        ...
        if (request.url === "/v1/execute") {
          ...
          executing = true;
          try {
            return await execute(root, body, request, options);
          } finally {
            executing = false;
          }
        }
      ...
      .catch((error: unknown) =>
        json(response, error instanceof RunnerHttpError ? error.status : 400, {
...
function write(
  ...
  if (
    ...
    content.byteLength > (options.maxFileBytes ?? 64 * 1024 * 1024)
  ) {
...
function read(
  ...
  const maxBytes = integer(
    record,
    "maxBytes",
    options.maxFileBytes ?? 64 * 1024 * 1024,
  );
...
async function execute(
  ...
  const timeoutMs = integer(
    record,
    "timeoutMs",
    options.maxTimeoutMs ?? 30 * 60_000,
  );
  const outputLimit = integer(
    record,
    "outputLimitBytes",
  ...
  request.once("close", () =>
    abort.abort(new Error("verification worker disconnected")),
  );
...
function run(
  ...
  return new Promise((resolveResult, reject) => {
    ...
    child.once("close", (code) => {
      ...
      cleanup();
      if (signal.aborted)
        return reject(signal.reason ?? new Error("verification aborted"));
      resolveResult({
...
function safeEnvironment(
  ...
  for (const [name, item] of Object.entries(value)) {
    ...
    if (item.length > 8192)
      throw new Error(`sandbox environment value is too large: ${name}`);
    result[name] = item;
  }
  return result;
}
...
function safePath(root: string, path: string, existing: boolean): string {
  if (
    ...
    throw new Error("sandbox path is not canonical and relative");
...
function rejectSymlinkAncestors(root: string, target: string): void {
  ...
  while (current !== root) {
    if (existsSync(current) && lstatSync(current).isSymbolicLink())
      throw new Error("sandbox path traverses symlink");
...
async function readJson(
  ...
  for await (const chunk of request) {
    ...
    if (size > maxBytes)
      throw new RunnerHttpError(413, "sandbox request is too large");
    chunks.push(bytes);
  }
  try {
    return JSON.parse(Buffer.concat(chunks).toString("utf8"));
  } catch (error) {
    throw new RunnerHttpError(400, "sandbox request must be JSON", {
      cause: error,
    });
  }
}
...
function object(value: unknown): Record<string, unknown> {
  if (value === null || typeof value !== "object" || Array.isArray(value))
    throw new Error("sandbox request must be an object");
...
function stringArray(record: Record<string, unknown>, key: string): string[] {
  ...
  if (
    ...
    value.length > 128 ||
    value.some(
      (item) => typeof item !== "string" || item === "" || item.length > 32_768,
    )
  )
...
async function main(): Promise<void> {
  ...
  const server = createVerificationSandboxRunnerServer({
    authToken,
    workspacePath:
      process.env["REEF_VERIFICATION_SANDBOX_WORKSPACE"] ?? "/workspace",
  });
...
function sandboxAuthToken(environment: NodeJS.ProcessEnv): string {
  ...
  if (
    ...
    throw new Error(
      "verification sandbox authentication configuration is incomplete",
    );
  }
