...
async function main(selected: string): Promise<void> {
  ...
  if (selected === "migrate") {
    const store = new PostgresVerificationStore(verificationPostgresConnectionFromEnvironment());
    try { await store.migrate(); process.stdout.write("verification migrations applied\n"); }
...
async function runApi(): Promise<void> {
  const store = new PostgresVerificationStore(verificationPostgresConnectionFromEnvironment());
  if (boolean(process.env, "REEF_VERIFICATION_AUTO_MIGRATE", true)) await store.migrate();
  ...
  const service = new VerificationService({ store, evidence: objects.evidence, profiles });
  ...
  const drain = (): void => {
    if (draining) return;
    draining = true;
    void publisher.drainOnce().catch((error: unknown) => {
      process.stderr.write(`verification dispatch failed: ${errorText(error)}\n`);
    }).finally(() => { draining = false; });
  };
  drain();
  const timer = setInterval(drain, integer(process.env, "REEF_VERIFICATION_DISPATCH_POLL_MS", 250));
  timer.unref();
  const handler = createVerificationHttpHandler(service, { readiness: () => store.ready() });
  ...
  await shutdown(); clearInterval(timer); await close(server); await store.close();
}

async function runWorker(): Promise<void> {
  const store = new PostgresVerificationStore(verificationPostgresConnectionFromEnvironment());
  if (boolean(process.env, "REEF_VERIFICATION_AUTO_MIGRATE", false)) await store.migrate();
  const profiles = loadTrustedVerificationProfiles();
  const objects = createProductionVerificationObjectStores();
  const worker = new DeterministicVerificationWorker({
    workerId: process.env["REEF_VERIFICATION_WORKER_ID"] ?? `verification-worker-${hostname()}-${randomUUID()}`,
    store,
    ...
    materializer: new DeterministicSourceBundleMaterializer({
      ...
      maxFileBytes: integer(process.env, "REEF_VERIFICATION_MAX_FILE_BYTES", 16 * 1024 * 1024),
      maxTotalBytes: integer(process.env, "REEF_VERIFICATION_MAX_TOTAL_BYTES", 512 * 1024 * 1024),
    }),
    ...
    maxInfrastructureRetries: integer(process.env, "REEF_VERIFICATION_MAX_INFRASTRUCTURE_RETRIES", 2),
  ...
  try {
    do {
      try {
        const worked = await worker.runOnce(); if (!worked && !once) await delay(pollMs);
      } catch (error) {
        if (once) throw error;
        process.stderr.write(`verification worker iteration failed: ${errorText(error)}\n`);
        await delay(pollMs);
      }
    } while (!once && !stopping);
  } finally { await store.close(); }
...
function listen(server: Server, host: string, port: number): Promise<void> {
  return new Promise((resolveListen, reject) => { server.once("error", reject); server.listen(port, host, resolveListen); });
}
function close(server: Server): Promise<void> {
  return new Promise((resolveClose, reject) => server.close((error) => error === undefined ? resolveClose() : reject(error)));
}
function shutdown(): Promise<void> {
  return new Promise((resolveStop) => { process.once("SIGINT", resolveStop); process.once("SIGTERM", resolveStop); });
}
