 ...
 */
const WEBVIEW_SCRIPT = `const vscode = acquireVsCodeApi();
...
const WEBVIEW_SCRIPT_HASH = createHash("sha256")
  .update(WEBVIEW_SCRIPT)
  .digest("base64");

export function webviewHtml(cspSource: string, _nonce: string): string {
  return `<!doctype html>
<html lang="en">
<head>
<meta charset="UTF-8" />
<meta http-equiv="Content-Security-Policy"
  content="default-src 'none'; style-src ${cspSource} 'unsafe-inline'; script-src 'sha256-${WEBVIEW_SCRIPT_HASH}';" />
<style>
